...
`assay extract` emits one JSON record per line. The extractor currently indexes Rust, Python, TypeScript, and TSX files while skipping hidden and generated directories.
...
```bash
repo=/path/to/repo
store="$HOME/.cache/assay/$(basename "$repo")"
mkdir -p "$store"
target/release/assay extract "$repo" \
  | STASH_DIR="$store" stash set
