...
impl BlockVisitor {
    pub fn new() -> Self {
        Self { depth: 0, decisions: 0, cognitive: 0, n1: 0, n2: 0 }
    }
...
impl<'ast> Visit<'ast> for BlockVisitor {
    ...
    fn visit_expr_binary(&mut self, node: &'ast ExprBinary) {
        use syn::BinOp;
        match node.op {
            BinOp::And(_) | BinOp::Or(_) => { self.decisions += 1; self.n1 += 1; }
            _ => { self.n1 += 1; }
        }
    ...
    fn visit_expr(&mut self, node: &'ast Expr) {
        match node {
            Expr::Try(_)  => { self.decisions += 1; self.n1 += 1; }
            Expr::Lit(_)  => { self.n2 += 1; }
            Expr::Path(_) => { self.n2 += 1; }
            _ => {}
        }
