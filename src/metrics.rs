...
pub fn compute(r: &RawCounts) -> FnMetrics {
    ...
    FnMetrics { name: r.name.clone(), line_start: r.line_start, line_end: r.line_end,
                loc, cc: r.decisions, cognitive: r.cognitive, halstead }
}

pub fn aggregate(path: &str, raw: &[RawCounts], fns: Vec<FnMetrics>, file_loc: usize) -> FileMetrics {
    let n = fns.len() as u32;

    // File CC = sum(fn CCs) - (N - 1): removes per-function base paths, keeps one for the file.
    let cc: u32 = fns.iter().map(|f| f.cc).sum::<u32>().saturating_sub(n.saturating_sub(1));
    ...
    FileMetrics { path: path.to_owned(), loc: file_loc, cc, cognitive, halstead, nmi, fns }
}
