...
fn rate_nmi(v: f64) -> &'static str {
    if v < 10.0      { RED }
    else if v < 20.0 { YELLOW }
    else             { GREEN }
}

fn worst(colours: &[&'static str]) -> &'static str {
    colours.iter().copied()
        .max_by_key(|&c| if c == RED { 2 } else if c == YELLOW { 1 } else { 0 })
        .unwrap_or(GREEN)
}
...
pub fn file_row(f: &FileMetrics) {
    ...
    table::row(&f.path, col,
        &f.loc.to_string(), &f.cc.to_string(),
        &f.cognitive.to_string(), &format!("{:.0}", f.halstead),
        &format!("{:.1}", f.nmi));
}
...
pub fn fn_row(m: &FnMetrics) {
    ...
    table::row(&label, col,
        &m.loc.to_string(), &m.cc.to_string(),
        &m.cognitive.to_string(), &format!("{:.0}", m.halstead), "─");
}

pub fn print_table(files: &[FileMetrics]) {
    use crate::table::{STATUS_W, LABEL_W, LOC_W, CC_W, COG_W, HV_W, NMI_W};
    println!("{}", table::top());
    println!(
    ...
    for (fi, f) in files.iter().enumerate() {
        if fi == 0 { println!("{}", table::mid()); }
        file_row(f);
        for m in &f.fns {
            println!("{}", table::mid());
            fn_row(m);
        }
        if fi < files.len() - 1 { println!("{}", table::dbl()); }
        else                     { println!("{}", table::bot()); }
    }
}
